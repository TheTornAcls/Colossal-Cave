...
// Lab 6, Part 1
//
// Added throw statements to SetChar and GetChar methods
//
...
void CMyString::SetChar(int iIndex, char ch)
{
    ...
    // greater than the length of the string then an exception is thrown
    if ((iIndex < m_iStringLen) && (iIndex >= 0))
    {
        ...
        throw CMyStringAccessException("CMyString Access violation in SetChar()");
}
...
char CMyString::GetChar(int iIndex) const
{
    ...
    // Throws a CMyStringAccessException if user tries to access past 
    // the logical end of the string
    if ((iIndex < m_iStringLen) && (iIndex >= 0))
        chLocal = m_pszBuffer[iIndex];
    else
        throw CMyStringAccessException();
