    ...
    LPARAM lParam)
{
    ...
    switch (message) {
        case WM_DROPFILES:
            DoDragDrop(hwnd, (HDROP)wParam, TRUE);
            break;
        case WM_SETCURSOR:
	...
	case WM_INTERIM:
        ...
        case WM_PAINT:
            BeginPaint(hwnd, &ps);
            CallWindowProc(lpEditWndProc, hEditWnd, message, (WPARAM)ps.hdc, (LPARAM)0);
            CardPaint(ps.hdc);
            EndPaint(hwnd, &ps);
            break;
        ...
        default:
PassMessageOn:
            return CallWindowProc(lpEditWndProc, hEditWnd, message, wParam, lParam);
    }
    return(0L);
}
    ...
    switch(wParam)
    {
        ...
        case VK_DELETE:
            if (EditMode == I_TEXT) /* should be handled by the edit control */
                ...
                BMKey(wParam);      /* we will handle this */
            return(TRUE);
    ...
    int ch)
{
    ...
    for (i = 0, iCard = iFirstCard+1; i < cCards; ++i, iCard++) {
        if (iCard == cCards) 
            iCard = 0;
        if ((char)(DWORD)AnsiUpper((LPSTR)(DWORD)(BYTE)*(Cards[iCard].line)) == ch)
            break;
    }
    GlobalUnlock(hCards);
    ...
    HDC hDC)
{
    ...
    {
        if (OleQueryBounds(CurCard.lpObject, &rc) != S_OK)
        {
            Hourglass(FALSE);
    ...
    int iCard)
{
    ...
    cCards--;
    Cards = (LPCARDHEADER) GlobalLock(hCards);
    RepMov(&Cards[iCard], &Cards[iCard+1], (cCards-iCard)*sizeof(CARDHEADER));
    GlobalUnlock(hCards);
...
int AddCurCard(void)
{
    ...
    if (i != cCards)
        RepMovDown(&Cards[i+1], &Cards[i], (cCards - i) * sizeof(CARDHEADER));
    ...
    char *pText)
{
    ...
    SendMessage(hEditWnd, WM_SETTEXT, 0, (LONG)(LPSTR)pText);
}
    ...
    int pos)
{
    ...
    if (iFirstCard != OldFirst)        /* did the above change anything? */
        PaintNewHeaders(hWindow);    /* yes, so redraw headers */
...
void DoPaste(int event) 
{
    if (EditMode == I_TEXT) {
        ...
        PicPaste(&CurCard, (event == PASTE), NULL);
}
    ...
    HWND hWnd)
{
    ...
    { 
        ...
        ExtTextOut(hDC, xCur+1, yCur+1+(ExtLeading/2),
            ETO_OPAQUE, &rect, lpTCards->line,
            lstrlen(lpTCards->line), NULL);
