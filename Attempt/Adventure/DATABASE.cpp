...
// Words may been represented in the list twice
// Searching with MAXWC will find one of the instances
// Searching the list with MAXWC will find the 2nd instance
...
int vocab(char *word,int val)
{
    ...
    if ((v1 = binary(word, g_wc, MAXWC)) >= 0)
    {
        ...
        //#ifdef DEBUG
        //printf("DATABASE.CPP vocab  v1 = %d, v2 = %d\n", v1,v2);
        //#endif

        if (v2 < 0)
        {
            //printf("DATABASE.CPP vocab v2 < 0\n");
            v2 = v1;
        }

        // is the minimum acceptable value 0??
        if (!val)
        {
            // if v1.acode < v2.acode
            //    return v1.acode
            // else
            //    return v2.acode
            //printf("DATABASE.CPP vocab val == %i\n",val);
            //printf("DATABASE.CPP vocab returning g_wc[v1].acode = %i\n",g_wc[v1].acode);

            return(g_wc[v1].acode < g_wc[v2].acode ? g_wc[v1].acode : g_wc[v2].acode);
        }
...
/*
    ...
    -1 indicates we are carrying the item
*/
int toting(int item)
{
    return(place[item] == -1);
...
void carry(int obj, int where)
{
    if (obj<MAXOBJ)
    {
        if (place[obj] == -1)
        {
            // we are already carrying the object
            return;
        }
...
void bug(int n)
{
    g_Printing.ConOutputError("Fatal error number %d\n", n);
    exit(1);
}
