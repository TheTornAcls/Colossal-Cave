...
void turn()
{
    ...
    */
    if ((newloc != g_iLoc) && !forced(g_iLoc) && ((cond[g_iLoc]&NOPIRAT)  ==  0))
...
#ifdef DEBUG1
        printf("FILE %s LINE %d loc = %d, g_iVerb = %d, g_iObject = %d, g_iMotion = %d\n",__FILE__,__LINE__, g_iLoc, g_iVerb, g_iObject, g_iMotion);
#endif

    if (g_iMotion)     /* execute player instructions	*/
        domove();
    else if (g_iObject)
        doobj();
    else
...
void describe()
{
    ...
    if (dark())
        rspeak(16);
    else if (visited[g_iLoc])
        descsh(g_iLoc);
    else
        desclg(g_iLoc);
    if (g_iLoc == 33 && pct(25) && !closing)
        rspeak(8);
}
...
void dotrav()
{
    ...
    for (kk = 0; travel[kk].tdest>=0 && !mvflag; ++kk)
    {
        ...
        if ((rverb != 1) && (rverb != g_iMotion) && !hitflag)
            continue;
...
void badmove()
{
    ...
    msg = 12;
    if (g_iMotion >= 43 && g_iMotion <=50) msg = 9;
    if (g_iMotion == 29 || g_iMotion == 30) msg = 9;
    if (g_iMotion == 7 || g_iMotion == 36 || g_iMotion == 37) msg = 10;
    if (g_iMotion == 11 || g_iMotion == 19) msg = 11;
    if (g_iVerb == FIND || g_iVerb == INVENTORY) msg = 59;
    if (g_iMotion == 62 || g_iMotion == 65) msg = 42;
    if (g_iMotion == 17) msg = 80;
    rspeak(msg);
}
...
void doobj()
{
    ...
    */
    if (fixed[g_iObject] == g_iLoc || here(g_iObject))
        trobj();
    /*
        did he give grate as destination?
    */
    else if (g_iObject == GRATE)
    {
        if (g_iLoc == 1 || g_iLoc == 4 || g_iLoc == 7)
        {
            g_iMotion = DEPRESSION;
            domove();
        }
        else if (g_iLoc>9 && g_iLoc<15)
        {
            g_iMotion = ENTRANCE;
            domove();
        }
    ...
    */
    else if ((liq() == g_iObject && here(BOTTLE)) ||
             liqloc(g_iLoc) == g_iObject)
        trobj();
    else if (g_iObject == PLANT && at(PLANT2) &&
             prop[PLANT2] == 0)
    {
        g_iObject = PLANT2;
        trobj();
    ...
    */
    else if (g_iObject == KNIFE && knfloc == g_iLoc)
    {
        rspeak(116);
        knfloc = -1;
    }
    ...
    */
    else if (g_iObject == ROD && here(ROD2))
    {
        g_iObject = ROD2;
        trobj();
    }
    else
        g_Printing.ConOutputError("I see no %s here.\n", probj() );
}
...
void trobj()
{
    if (g_iVerb)
        ...
        g_Printing.ConOutput("What do you want to do with the %s?\n",probj() );
}
...
char * probj()
{
    int wtype, wval;
    #ifdef DEBUG1
    printf("TURN.cpp probj calling analyze\n");
    #endif
...
void dwarves()
{
    int i, j, k, iTry, attack, stick, dtotal;
    /*
    ...
    */
    ...
    if (dtotal == 0)
        return;
    if (dtotal > 1)
        g_Printing.ConOutputError("There are %d threatening little dwarves in the room with you!\n", dtotal);
    else
        rspeak(4);
    if (attack == 0)
        return;
    ...
    if (attack > 1)
    {
        g_Printing.ConOutputError("%d of them throw knives at you!!\n", attack);
        k = 6;
    }
    else
    {
    ...
    if (stick <= 1)
    {
        ...
        g_Printing.ConOutputError("%d of them get you !!!\n", stick);
    oldloc2 = newloc;
    death();
}
/*
...
int stimer()
{
    ...
    if (tally == 0 && g_iLoc >= 15 && g_iLoc != 33)
        --clock1;
    if (clock1 == 0)
    {
        /*
